...
The final result I achieve is __%__. However, there's _no garantee_ that the model still obtains similar performance on the test set since I use extended data to train the classifier. [Source code]() (written in C++ and tested on Visual Studio 2010) is available for reference. 
