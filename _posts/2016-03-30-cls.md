...
The final result I achieve is __around 71%__ ('around' is due to randomness in selecting descriptors in dense sampling during each experiment). However, there's _no garantee_ that the model still obtains similar performance on the test set since I use extended data to train the classifier. [Source code]() (written in C++ and tested on Visual Studio 2010) is available for reference. 
...
<table class="tg">
  ...
  <tr>
    <td class="tg-baqh"><b>~71%</b> (final)</td>
